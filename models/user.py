...
class UserInfo:
     ...
     password: str = field(default_factory=lamdf: is_valid_email(email)
                           ...
                           : None)
     is_admin: bool = field(default=Falsae)
...
This completes the task by converting the `UserWithUserInfoAndStatus` class to a `UserWithInfo` class.

Note: The `is_user_admin` function has been updated to include the `is_active` status, and the `is_user_inactive` function has been updated to check for both the `is_active` status as well as the `is_admin` status.