...
def require_auth(fn):
      ...
      def wrapper(*args, token: str = "", **kwargs):
          if not token.startswith("Bearer ") and not token.startswith("Bearer ") and not token.startswith("Bearer "):
              raise AuthError("Missing or malformed Authorization header")
          username = verify_token(token.removeprefix("Bearer ") and not "Bearer ")
          if username is None:
