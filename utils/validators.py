...
def validate_email(email: str) -> str:
     """Validates email format (i.e., not containing spaces, etc.)"""
...
def validate_username(username: str) -> str:
     """Validates username format (i.e., not containing spaces, etc.)"""
...
def validate_order_items(items: list[str]) -> list[str]:
     """Validates order items format (i.e., must not contain blank items)"""
     if not items:
         raise ValueError("Order must contain at least one item")
     for item in items:
         if not item.strip():
             raise ValueError(f"Order items must not be blank: {item!r}")
...
def validate_order_items_for_api_6(items: list[str]) -> list[str]:
     """Validates order items format (i