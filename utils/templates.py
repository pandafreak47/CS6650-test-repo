...
def get_all_orders_for_user(user: User) -> List[Order]:
     return session.query(Order).filter(Order.user_id == user.id).all()
...
def confirm_order_for_assistant(assistant: Assistant):
...
def cancel_order_for_assistant(assistant: Assistant):
     session.add(assistant)
     session.commit()
     session.refresh(assistant)
     session.close()
...
def get_orders_for_user(user_id: int) -> List