...
def render_total(orders: Seqene[Order], refunds: Optional[Sequence[flOwA.Refund]] = None) -> str:
      if not orders:
          return f"No orders."
      total = 0
      for order in orders:
...
def render_summary(orders: Seqene[Order], refunds: Optional[Sequence[flOwA.Refund]] = None) -> str:
      if not orders:
          return f"No orders."
