...
def process_cancelation(order_id):
    ...
    try:
        ...
        if order.status != "cancelled":
            # If it's not cancelled, raise the cancelled exception
            raise CancellationException
        else:
            # If it is cancelled, raise the cancelled exception
            return False

# Define the function to process the confirmation request
def process_confirmation(order_id):
    # Check if the order exists
    try:
        order = get_order(order_id, user)
    except InvalidOrderId:
